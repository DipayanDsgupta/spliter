...
export default function AuthPage() {
    const { completeProfile, isSupabaseConfigured, currentUser, needsProfile } = useApp()
    const navigate = useNavigate()

    /* Auto-redirect if already logged in */
    useEffect(() => {
        if (currentUser) {
            navigate(needsProfile ? '/profile-setup' : '/', { replace: true })
        }
    }, [currentUser, needsProfile, navigate])
    ...
    /* ── Google Sign-In (Commented as requested) ── */
    /*
    ...
    */
    ...
    const handleVerifyOtp = async () => {
        const token = otp.join('').replace(/\D/g, '')
        ...
        setLoading(false)
        if (err) { setError(`Invalid OTP: ${err.message || 'Please try again'}`); return }
        // Note: Global state will handle the redirect to Profile or Dashboard
    }
        ...
        if (v.length > 1) {
            ...
            if (pasted.length > 0) {
                ...
                pasted.forEach((digit, index) => {
                    if (i + index < 6) next[i + index] = digit;
                });
                setOtp(next);
                // Auto-focus next input or last
                const nextFocusIndex = Math.min(i + pasted.length, 5);
                ...
                <AnimatePresence mode="wait">
                    ...
                    {step === STEPS.LANDING && (
                        <motion.div key="landing" {...slide}>
                            ...
                            <p className="text-sm mb-5" style={{ color: '#94A3B8' }}>Sign in with email OTP</p>
                    ...
                    {step === STEPS.EMAIL_OTP && (
                        <motion.div key="email-otp" {...slide}>
                            ...
                            <p className="text-center text-xs mt-3" style={{ color: '#475569' }}>OTP sent via Gmail Secure Mail</p>
                        </motion.div>
                    )}
